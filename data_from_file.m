% Implementation of the KALMAN filter on ECG recordings from MIT/BIH database.
% The first section of the code reads the ECG recordings from the MIT/BIH
% database which are stored in a special file format (212 format).
% The second section of the code processes the ECG data obtained from the
% database and performs the QRS detection. The output of the QRS detection
% is a train of impulses, with impulses located at the occurrence of a QRS
% complex in the ECG. Finally, the RR intervals are computed from
% this data and passed on to the KALMAN filter algorithm for estimation.
...
ATRFILE= '102.atr'; % attributes-file in binary format
DATAFILE='102.dat'; % data-file
...
t=0;
N=37; % this is the width of the integration window
...
b=0;
delay=0;
k=2; %filter peak variable
d=2; %integrator peak variable
...
%             fprintf(1,' 11231321 %d\n', qmax(1,k));
        if i==500+35 %this corresponds to 2seconds(the largest time needed for 1 beat
            [a t]=max(qmax(1,:)); %finds maximum point after several max locations
            n=1;
            for m=1:length(qmax)
